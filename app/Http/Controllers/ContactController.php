...
class ContactController extends Controller
{
    ...
    protected function send(Request $request)
    {
        ...
        $check = contact_us::where([['name_family', '=', $request['name_family']], ['massage', '=', $request['massage']]])->first();
        if(!empty($check) ){
            return redirect()->back()->with('check', 'IT WORKS!');
        } else{
            $contact = new contact_us($request->all());
            $contact->name_family = $request['name_family'];
            $contact->subject = $request['subject'];
            $contact->massage = $request['massage'];
            $contact->mobile = $request['mobile'];
            $contact->email = $request['email'];
            $contact->date = $v->formatDate('Y/M/D') ;
            $contact->time = $v->formatTime() ;
            $contact->ip = $ip;
            $contact->save();
